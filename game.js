...
// Popola oggetti iniziali SOLO se entri in main (non allo start)
...
function claimQuest(){
  var q=quests[currentQuest]; if(!q||q.status!=='completed') return; var rw=q.reward||{};
  if(rw.xp) gainXP(rw.xp); if(rw.pots) player.pots += rw.pots; q.status='claimed';
  toast('Ricompensa ottenuta.');
  logLoot('Ricompensa: +'+(rw.xp||0)+' XP'+(rw.pots?(', +'+rw.pots+' poz.'):'')); renderQuest(); updateHotbarCounts();
}
...
function onCanvasTap(ev){
  try{
    ...
    // Clic su nemico = target / auto-chase
    ...
    // Attacco rapido se già adiacente
...
btnLoad.addEventListener('click', function(){ try{
  ...
  draw(); 
  drawMini();
  if (typeof renderInventoryIfOpen === 'function') {
    renderInventoryIfOpen();
  }
  updateTargetFrame();
  updateDpsPanel();
...
function draw(){
  ...
  for(var pp=0; pp<projectiles.length; pp++){ var pr=projectiles[pp]; var ptx = (pr.vx!=null?pr.vx:pr.x), pty=(pr.vy!=null?pr.vy:pr.y);
